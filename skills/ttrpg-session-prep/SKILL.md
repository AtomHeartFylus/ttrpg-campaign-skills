...
2. **The official module/chapter** for this session, if any (`D.backbone`, `D.official_material`).
   ...
   text enters it as a citation for use at your own table, never for redistribution — `SECURITY.md`
   states the package's position; `C.player_access` must never expose it to a player who has not
   bought the source it came from.
