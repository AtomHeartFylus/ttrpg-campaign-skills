...
**Gate 2 — `B.consent_offgame`, for Phase 5 only.** Agreeing to be recorded is **not** agreeing to a
durable, themed, timecoded index of your out-of-character talk: the first is a file nobody reads, the
second is a document about named people that outlives the evening.
...
**`E.overrides` branch — mandatory.** This is the least principle-heavy skill in the package: it
applies exactly **one** overridable strong default, so read the slot in Phase 0 and drop that
default if the table switched it off. Enforcing a default the profile has switched off is as wrong
as inventing a slot value.

| Override | What stops being required here |
|---|---|
| `P13 — off` | the admission test on what the off-game half produces: a hook born off game enters the campaign without answering the three questions, and Phase 5 keeps it on the strength of its timecode alone |

No other strong default governs this skill, so the slot has nothing else to switch off here.
...
Two classes of file with opposite handling. Get it wrong once and the repository is permanently heavy.
...
re-derive a layout the profile already fixes. If the slot is empty, propose the conventions below,
ask before creating anything, and write the accepted answer back into `C.capture_paths`:
...
- Both transcript files carry **the same base name as the audio file**, in a sibling transcript
  folder, and so does the speaker-map note. Same name = the set is self-evident a year later.
  File names follow `C.naming`, and any heading inside those notes is written in `B.language`.
...
- *Example of a local pipeline (not a requirement):* an open-source speech-to-text model plus a
  speaker-diarization stage, run locally from a separate project directory by a small script that
  emits both files. Any equivalent — including a hosted service — is fine, provided
  `E.never_without_asking` and `E.overrides` permit sending the table's audio to a third party and
  `B.consent_recording` covers it too.
...
may be split across two labels. Therefore:

- Rebuild the map **every session**, by sampling a few timecodes per label until each is
  identified.
- Record it in **its own note beside the transcript pair**, sharing the base name of the audio, at
  the path `C.capture_paths` declares — and nowhere else. It is a fact about *that file*, not about
  the campaign (P10), and not a record of what happened: `ttrpg-session-log` owns the log, whose
  skeleton has no room for it. A global "speaker table" in an index is wrong for the same reason.
- The log **reads** this note when it needs to attribute a moment; it never copies the table in.
- Mark labels you could not resolve as unresolved. Never guess one to complete the map.

```markdown
# Session N — Speaker map                      <!-- blocks per C.blocks, headings in B.language -->

> Valid for this transcript only. Labels are positional and do not carry across files.
...
- **Where it lives is read from `C.player_access` / `C.gm_private`, never improvised.** With
  players able to read the repo it goes to the GM-private location or is agreed openly with the
  table; if `C.gm_private` is empty while players have access, stop and ask. **Those two decide,
  `C.capture_paths` only supplies a path that obeys them** — on a conflict the access rule wins and
  you stop and ask.
- **Curate, do not transcribe:** themes with timecodes, not quotes — and crude, offensive or
  personal material is indexed by timecode and never quoted.
- **Any participant may have any entry removed, without discussion or a reason**, which is why
  entries are short pointers: a pointer deletes cleanly. State the rule in the note's own header.
...
- The speaker map is its own note beside the transcript pair, shares the base name of the audio,
  and marks unresolved labels as unresolved. It is **not** inside the session log.
- If it exists: the off-game note is a curation, not a transcript, quotes no crude or personal
  material, sits where `C.player_access` / `C.gm_private` permit, and carries the removal rule in
  its own header.
...
- Do not start, continue or resume a capture pipeline while `B.consent_recording` is empty, `no`
  or ambiguous, and never infer consent from the existence of an audio file — nor read anything in
  `E.overrides` as permission to skip either gate.
...
- Do not re-derive a folder layout `C.capture_paths` declares, or improvise where a GM note lives.
- Do not commit the audio, or any multi-megabyte intermediate output of the transcription tool.
- Do not name a specific tool or environment as a requirement in campaign notes — record the output
  contract (readable + timecoded, diarized) and the invocation, nothing more.
...
- Do not reuse last session's speaker map, put it anywhere but its own note beside that session's
  transcript (not in the log, not in an index), or guess an unresolved speaker.
- Do not treat the transcript as minutes, quote it into the log, or copy names out of it.
- Do not turn the off-game note into a second transcript, quote crude, offensive or personal
  material in it (index it by timecode), or write an entry that serves neither the table's mood nor
  a hook.
